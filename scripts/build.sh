...
install_zulu() {  # $1=java_version  $2=hedef .jdk
	...
	# NOT: Azul metadata v1 uçnoktası "features=" parametresini YOK SAYAR (ölçüldü:
	# features=shenandoah, features=fx ve uydurma bir features=zzz aynı paketi döndürüyor)
	# → belirli bir GC/özelliği indirme anında garanti etmenin yolu yok. Garanti package()
	# içindeki runtime testiyle sağlanır (Shenandoah desteklenmiyorsa bayrak hiç eklenmez).
	local url
