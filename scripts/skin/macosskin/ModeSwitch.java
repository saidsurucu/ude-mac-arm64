...
public final class ModeSwitch {
    ...
    public static void apply(String mode) {
        try {
            ...
            // Tema değişimi (setSkin + updateComponentTreeUI) klavye odağını
            // editörden alıp ribbon/konteyner bileşenine (gui.iA) taşıyor →
            // Cmd biçimlendirme kısayolları (MacShortcutRemap sentetik Ctrl'ü
            // ODAK SAHİBİNE gönderir) editöre ulaşmıyor; kullanıcı metne
            // tıklayana dek "kısayollar bozuldu" görünüyor. Odağı aktif
            // pencerenin editörüne geri ver. invokeLater: combo'nun kendi
            // (asenkron) odak transferi bittikten SONRA çalışsın, yoksa bizim
            // requestFocus'u ezer.
    ...
    /** Aktif pencerenin editörüne (text.hj türevi) odağı geri ver. Odak
     *  zaten bir metin alanındaysa (editör ya da arama kutusu) dokunma —
     *  yalnız tema değişiminin editör-dışı konteynere (gui.iA) kaçırdığı
     *  odağı toparlar. */
